...
(defcustom ledger-reports
  '(("bal" "ledger -f %(ledger-file) bal")
    ...
    ("pnl-as-of" "ledger -f %(ledger-file) -s -e %(date) bal ^assets ^debts"))
...
The user is prompted to enter a date and that is substituted. If
point is in an entry, the date for that entry is used as the default"
  (ledger-read-string-with-default "Date" "mm/dd"))
...
This is done so that the decimal falls in colum 40"
  (interactive "p")
